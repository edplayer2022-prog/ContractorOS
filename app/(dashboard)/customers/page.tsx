...
export default async function CustomersPage() {
  ...
  return <>
    <PageHeader title="Customers" description="Manage customers and their job sites." action={hasPermission(membership?.role,"customers.create")&&<Link href="/customers/new"><Button><Plus className="h-4 w-4"/>New Customer</Button></Link>}/>
    <Card className="overflow-hidden">
      {!customers?.length ? <EmptyState icon={<Users className="h-6 w-6"/>} title="No customers yet" description="Add your first customer to start building an estimate." action={hasPermission(membership?.role,"customers.create")&&<Link href="/customers/new"><Button><Plus className="h-4 w-4"/>Add customer</Button></Link>}/> :
