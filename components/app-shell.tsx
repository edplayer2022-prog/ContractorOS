...
export function AppShell({ companyName, ownerName,role,permissions, children }: { companyName: string; ownerName: string;role:MemberRole;permissions:readonly Permission[]; children: React.ReactNode }) {
  const path = usePathname(); const [open,setOpen]=useState(false);
  const sidebar = <><div className="flex h-16 items-center gap-3 border-b border-white/10 px-5"><span className="grid h-9 w-9 place-items-center rounded-lg bg-orange-500"><HardHat className="h-5 w-5"/></span><span className="text-lg font-bold">ContractorOS</span></div><nav className="flex-1 space-y-1 overflow-y-auto p-3">{links.filter(link=>permissions.includes(link.permission)).map(({href,label,icon:Icon})=><Link key={href} href={href} onClick={()=>setOpen(false)} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition", path===href || (href!=="/dashboard"&&path.startsWith(href)) ? "bg-white/10 text-white" : "text-slate-300 hover:bg-white/5 hover:text-white")}><Icon className="h-5 w-5"/>{label}</Link>)}</nav><div className="border-t border-white/10 p-3"><div className="mb-2 flex items-center gap-3 rounded-lg px-2 py-2"><span className="grid h-9 w-9 place-items-center rounded-full bg-orange-500/20 text-xs font-bold text-orange-300">{initials(companyName)}</span><div className="min-w-0"><p className="truncate text-sm font-semibold">{companyName}</p><p className="truncate text-xs text-slate-400">{ownerName} · {role.replaceAll("_"," ")}</p></div></div><form action={logout}><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white"><LogOut className="h-4 w-4"/>Log out</button></form></div></>;
