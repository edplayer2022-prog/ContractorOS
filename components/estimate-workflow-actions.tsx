...
export function EstimateWorkflowActions({id,status:initialStatus,token:initialToken,customerEmail}:{id:string;status:EstimateStatus;token:string|null;customerEmail?:string|null}){
  ...
  return <div className="flex flex-wrap items-center gap-2">
    ...
    {status==="approved"&&<><Can permission="projects.create"><CreateProjectButton estimateId={id}/></Can><Can permission="invoices.create"><Link href={`/invoices/new?estimate=${id}&type=deposit`}><Button variant="secondary">Create Deposit Invoice</Button></Link></Can><Can permission="invoices.create"><Link href={`/invoices/new?estimate=${id}&type=custom&import=1`}><Button variant="secondary">Create Invoice</Button></Link></Can><Can permission="change_orders.create"><Link href={`/change-orders/new?estimate=${id}`}><Button variant="secondary"><FilePlus2 className="h-4 w-4"/>Create Change Order</Button></Link></Can></>}
    {(status==="approved"||status==="rejected")&&<Can permission="estimates.create"><Button variant="secondary" onClick={duplicate} loading={loading}><Copy className="h-4 w-4"/>{status==="rejected"?"Create Revised Estimate":"Duplicate"}</Button></Can>}
